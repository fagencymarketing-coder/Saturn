...
(async () => {
  ...
  const base = 'file://'+process.cwd()+'/site-prototype-v4.html';
  // desktop screenshot
  ...
  await p.screenshot({path:'v4-desk.png', fullPage:true});
  // mobile screenshot
  ...
  await m.screenshot({path:'v4-mob.png', fullPage:true});
  // PDF
  ...
  await pp.pdf({path:'Saturn-prototype-v4.pdf', width:'1280px', height:H+'px', printBackground:true, pageRanges:'1'});
  console.log('pdf H=',H);
  await b.close();
})();
