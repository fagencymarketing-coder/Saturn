...
/* ===== V5: компактная главная ===== */
/* блок "Что вам требуется" — вкладки */
.need-tabs{display:flex;flex-wrap:wrap;gap:10px;margin-top:26px}
.need-tab{display:inline-flex;align-items:center;gap:9px;font-family:inherit;font-size:14.5px;font-weight:600;cursor:pointer;
  border:1.5px solid var(--line-l);background:var(--white);color:var(--text-l);border-radius:12px;padding:12px 18px;transition:.15s}
.need-tab .ic{width:22px;height:22px;color:var(--green)}
.need-tab:hover{border-color:var(--green)}
.need-tab.on{background:var(--ink);border-color:var(--ink);color:#fff}
.need-tab.on .ic{color:var(--orange-soft)}
.need-panel{display:none;margin-top:22px}
.need-panel.on{display:block}
.need-simple{background:var(--white);border:1px solid var(--line-l);border-radius:16px;padding:26px 28px;border-top:4px solid var(--green)}
.need-simple p{font-size:15.5px;color:var(--muted-l);max-width:70ch;margin:0 0 18px}
.need-simple .btn{margin-right:12px}
/* каталог: 3 карточки + CTA */
.grid-3{grid-template-columns:repeat(3,1fr)}
.cat-cta{margin-top:24px}
/* сервис: CTA-строка */
.svc-cta{display:flex;align-items:center;gap:14px;flex-wrap:wrap;margin-top:30px}
.svc-cta .podbor-alt{color:var(--muted-l);font-size:14px}
.svc-cta .podbor-alt a{color:var(--green-deep);font-weight:700}
/* компания + география + производители */
.cg{display:grid;grid-template-columns:1fr 1.15fr;gap:30px;margin-top:28px;align-items:start}
.cg-facts{display:grid;gap:12px;align-content:start}
.cg-facts .about-btns{margin-top:8px}
.cg-map{display:grid;grid-template-columns:1fr;gap:16px}
.cg-map .regions{display:grid;grid-template-columns:1fr 1fr;gap:10px}
.cg-map .mapbox{background:linear-gradient(135deg,#241F1A,#17130F);border:1px solid var(--line-d);border-radius:16px;min-height:120px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;color:var(--muted-d2);text-align:center;padding:20px}
.cg-map .mapbox .big{font-family:"Archivo";font-weight:800;font-size:44px;color:var(--green-soft);line-height:1}
.prod-row{margin-top:34px;padding-top:26px;border-top:1px solid var(--line-d);display:flex;align-items:center;gap:26px;flex-wrap:wrap}
.prod-h{font-family:"IBM Plex Mono";font-size:12px;letter-spacing:.12em;text-transform:uppercase;color:var(--muted-d2)}
.prod-logos{display:flex;gap:14px;flex-wrap:wrap}
.plogo{font-family:"Archivo";font-weight:800;font-size:17px;color:var(--text-d);background:rgba(255,255,255,.05);border:1px solid var(--line-d);border-radius:10px;padding:10px 18px}
/* подтверждаем результат: 2 колонки */
.proof{display:grid;grid-template-columns:1fr 1fr;gap:24px;margin-top:26px}
.proof-col{background:var(--white);border:1px solid var(--line-l);border-radius:16px;padding:26px 28px}
.proof-h{font-family:"Archivo";font-weight:800;font-size:18px;margin-bottom:16px}
.proof-res{display:grid;gap:14px}
.pres{display:flex;gap:16px;align-items:baseline}
.pres b{font-family:"Archivo";font-weight:800;font-size:30px;color:var(--green-deep);flex:none;min-width:88px}
.pres span{font-size:13.5px;color:var(--muted-l)}
.proof-docs{list-style:none;margin:0 0 16px;padding:0;display:grid;gap:11px}
.proof-docs li{font-size:14.5px;font-weight:500;border-bottom:1px solid var(--line-l);padding-bottom:11px}
.proof .plink{color:var(--green-deep)}
.proof .cat-note{margin-top:16px}
/* контакт + форма */
.contact{display:grid;grid-template-columns:1fr 1.2fr;gap:24px;margin-top:28px}
.contact-person{background:var(--ink-soft);border:1px solid var(--line-d);border-radius:16px;padding:26px}
.cp-top{display:flex;gap:16px;align-items:center}
.cp-top .av{width:70px;height:70px;border-radius:14px;background:#EFE9E1;border:1px solid #E0D8CB;flex:none;display:grid;place-items:center;color:#8A8078;font-size:12px}
.cp-top h3{font-size:19px}
.cp-top .role{font-size:13px;color:var(--muted-d);margin-top:3px}
.cp-text{color:var(--muted-d);font-size:14.5px;margin:16px 0 16px}
.cp-lines{display:grid;gap:8px;margin-bottom:16px}
.cp-lines a{font-size:15px;color:var(--text-d)}
.cp-lines .cp-tel{font-family:"IBM Plex Mono";font-weight:600;color:var(--orange-soft)}
...
@media(max-width:980px){ .cg,.proof,.contact{grid-template-columns:1fr} .grid-3{grid-template-columns:1fr 1fr} }
@media(max-width:620px){ .grid-3,.cg-map .regions{grid-template-columns:1fr} .pres b{min-width:70px;font-size:26px} }
</style>'''
...
<header class="hdr"><div class="wrap in">
  ...
  <nav class="nav">
    <a href="#catalog">Каталог</a><a href="#dirs">Подбор</a><a href="#service">Диагностика</a>
    <a href="#service">Агросопровождение</a><a href="#about">О компании</a><a href="#contact">Контакты</a>
  </nav>
  <div class="right"><span class="tel">+7 960 953-48-88</span><a class="btn o sm" href="#contact">Получить предложение</a></div>
</div></header>

<!-- 1. HERO (тёмный) -->
<section class="hero">{HERO_WAVES}
  <div class="wrap in">
    <span class="status"><span></span>Оптовые поставки для сельхозпроизводителей</span>
    ...
    <div class="trust">Оптовые поставки<span class="dot"></span>8 регионов присутствия<span class="dot"></span>сопровождение на всех этапах</div>
  </div>
</section>

<!-- 2. ЧТО ВАМ ТРЕБУЕТСЯ? (направления + подбор, интерактив) (светлый) -->
<section class="sec sec--light" id="dirs"><div class="wrap">
  <div class="eyebrow">Подбор</div>
  <h2 class="sec-h">Что вам требуется?</h2>
  <p class="sec-lead">Выберите направление — подберём продукцию под культуру и задачу или сразу свяжем со специалистом.</p>
  <div class="need-tabs" role="tablist">
    <button class="need-tab on" data-t="prod" data-name="Удобрения">{icon('udobreniya',22)}Удобрения</button>
    <button class="need-tab" data-t="prod" data-name="СЗР">{icon('szr',22)}СЗР</button>
    <button class="need-tab" data-t="prod" data-name="Семена">{icon('semena',22)}Семена</button>
    <button class="need-tab" data-t="diag">{icon('diagnostika',22)}Диагностика</button>
    <button class="need-tab" data-t="agro">{icon('agro',22)}Агросопровождение</button>
  </div>

  <div class="need-panel on" id="np-prod">
    <div class="podbor">
      <div class="pcard">
        <div class="lab">По культуре</div>
        <div class="chips3">
          <span class="chip3">{icon('zernovye',18)}Зерновые</span>
          <span class="chip3">{icon('kukuruza',18)}Кукуруза</span>
          <span class="chip3">{icon('podsolnechnik',18)}Подсолнечник</span>
          <span class="chip3">{icon('zernobob',18)}Зернобобовые</span>
          <span class="chip3">Соя</span><span class="chip3">Рапс</span><span class="chip3">Свёкла</span>
        </div>
      </div>
      <div class="pcard">
        <div class="lab">По задаче</div>
        <div class="chips3">
          <span class="chip3 g">Стартовое питание</span><span class="chip3 g">Листовая подкормка</span>
          <span class="chip3 g">Основное внесение</span><span class="chip3 g">Обработка семян</span>
          <span class="chip3 g">Коррекция дефицита</span><span class="chip3 g">Защита</span>
          <span class="chip3 g">Антистресс</span>
        </div>
      </div>
    </div>
    <div class="podbor-act">
      <a class="btn o" href="#catalog">Показать подходящие продукты</a>
      <span class="podbor-alt">Не знаете, что выбрать? <a href="#contact">Спросить агронома →</a></span>
    </div>
  </div>

  <div class="need-panel" id="np-diag">
    <div class="need-simple">
      <p>Функциональная листовая диагностика: обследуем посевы, определяем дефициты питания и корректируем схему под конкретное поле.</p>
      <a class="btn g" href="#contact">Заказать диагностику</a>
      <a class="btn ghost sm" href="#service">Подробнее о диагностике →</a>
    </div>
  </div>
  <div class="need-panel" id="np-agro">
    <div class="need-simple">
      <p>Ведём культуру весь сезон: подбор продукции, разработка схемы, расчёт норм, консультации и контроль состояния посевов.</p>
      <a class="btn g" href="#contact">Обсудить задачу с агрономом</a>
      <a class="btn ghost sm" href="#service">Подробнее об агросопровождении →</a>
    </div>
  </div>
</div></section>
...
<section class="sec sec--dark" id="catalog"><div class="wrap">
  <div class="sec-head-row">
    <div><div class="eyebrow">Каталог</div><h2 class="sec-h">Популярные позиции</h2></div>
    <a class="btn ghost">Весь каталог →</a>
  </div>
  <p class="sec-lead">По одному примеру из каждого раздела. Полный ассортимент и фильтры — на странице каталога.</p>
  <div class="grid grid-3">
    <div class="card"><div class="img">Фото продукта</div><h3>Фертика Листовое Старт 13-40-13</h3><div class="meta">Фертика · удобрение</div><div class="price">от 227 ₽/кг</div><div class="row"><a class="btn ghost sm">Подробнее</a><a class="btn o sm">Рассчитать поставку</a></div></div>
    ...
    <div class="card"><div class="img">Фото продукта</div><h3>Семена (пример)</h3><div class="meta">семена · уточняется</div><div class="price">Цена по запросу</div><div class="row"><a class="btn ghost sm">Подробнее</a><a class="btn o sm">Получить предложение</a></div></div>
  </div>
  <div class="cat-cta"><a class="btn o" href="#">Перейти в каталог</a></div>
  <p class="cat-note">Итоговая стоимость зависит от объёма, региона и условий поставки. Для части продукции доставка, хранение и сопровождение включаются в предложение.</p>
</div></section>

<!-- 5. СЕРВИС В 4 ЭТАПА + ДИАГНОСТИКА/АГРОСОПРОВОЖДЕНИЕ (светлый) -->
<section class="sec sec--light" id="service"><div class="wrap">
  <div class="eyebrow">Сервис под ключ</div>
  <h2 class="sec-h">Подбор — диагностика — поставка — сопровождение</h2>
  <p class="sec-lead">«Сатурн» — не просто продажа продукции. Мы ведём хозяйство на всех этапах сезона.</p>
  <div class="steps">
    <div class="step"><div class="n">1</div><h3>Подбор</h3><p>Консультация агронома, подбор продукции под культуру и регион, расчёт норм.</p></div>
    <div class="step"><div class="n">2</div><h3>Диагностика</h3><p>Листовая диагностика и мониторинг посевов, выявление дефицитов питания.</p></div>
    <div class="step"><div class="n">3</div><h3>Поставка и хранение</h3><p>Организация поставки, хранение и отгрузка оптом в ваш регион.</p></div>
    <div class="step"><div class="n">4</div><h3>Сопровождение</h3><p>Контроль состояния посевов и корректировка программы в течение сезона.</p></div>
  </div>

  <div class="svc-cta">
    <a class="btn g" href="#contact">Заказать диагностику</a>
    <a class="btn g" href="#contact">Обсудить задачу с агрономом</a>
    <span class="podbor-alt">Подробнее — на страницах <a href="#service">«Диагностика»</a> и <a href="#service">«Агросопровождение»</a></span>
  </div>
</div></section>

<!-- 5. КОМПАНИЯ + ГЕОГРАФИЯ + ПРОИЗВОДИТЕЛИ (тёмный) -->
<section class="sec sec--dark" id="about"><div class="wrap">
  ...
  <p class="sec-lead">Обеспечиваем сельхозпроизводителей семенами, удобрениями и средствами защиты растений: подбираем продукцию под культуру и регион, организуем поставку и сопровождаем применение. Головной офис — в Барнауле, работаем в 8 регионах России.</p>
  <div class="cg">
    <div class="cg-facts">
      <div class="af"><span>Юридическое лицо</span><b>ООО «Сатурн»</b></div>
      <div class="af"><span>Головной офис</span><b>г. Барнаул</b></div>
      <div class="af"><span>География</span><b>8 регионов регулярных поставок</b></div>
      <div class="about-btns"><a class="btn ghost">Подробнее о компании</a><a class="btn ghost">Скачать реквизиты</a></div>
    </div>
    <div class="cg-map">
      <div class="regions">{regions_html}</div>
      <div class="mapbox"><span class="big">8</span>регионов поставок<br><span class="mapleg">● офис — Барнаул &nbsp; ● регионы поставок &nbsp; ○ по запросу</span></div>
    </div>
  </div>
  <div class="prod-row">
    <div class="prod-h">Продукция российских производителей</div>
    <div class="prod-logos">
      <span class="plogo">Фертика</span><span class="plogo">Волски Биохим</span><span class="plogo">Relict Organics</span>
    </div>
  </div>
</div></section>

<!-- 9. УСЛОВИЯ СОТРУДНИЧЕСТВА (тёмный) -->
<section class="sec sec--dark" id="terms"><div class="wrap">
  <div class="eyebrow">Для закупщика</div>
  <h2 class="sec-h">Условия сотрудничества</h2>
  <p class="sec-lead">Работаем с хозяйствами оптом — прозрачно по срокам, доставке и оплате.</p>
  <div class="terms">
    <div class="term">{icon('udobreniya',30)}<h3>Оптовые поставки</h3><p>Минимальные партии и формат заказа — уточняются под задачу.</p></div>
    <div class="term">{icon('agro',30)}<h3>Доставка до хозяйства</h3><p>Расчёт по региону и объёму поставки.</p></div>
    <div class="term">{icon('semena',30)}<h3>Хранение и комплектация</h3><p>Своевременная комплектация и отгрузка партий.</p></div>
    <div class="term">{icon('diagnostika',30)}<h3>Гибкие условия оплаты</h3><p>После индивидуального согласования.</p></div>
  </div>
</div></section>

<!-- 7. ПОДТВЕРЖДАЕМ РЕЗУЛЬТАТ (результаты + документы) (светлый) -->
<section class="sec sec--light" id="proof"><div class="wrap">
  <div class="eyebrow">Доверие</div>
  <h2 class="sec-h">Подтверждаем результат</h2>
  <div class="proof">
    <div class="proof-col">
      <div class="proof-h">Результаты испытаний</div>
      <div class="proof-res">
        <div class="pres"><b>до +15%</b><span>пшеница — результат производственного испытания в конкретных условиях хозяйства</span></div>
        <div class="pres"><b>+4%</b><span>соя — результат испытания продукции в конкретном хозяйстве</span></div>
      </div>
      <a class="plink">Смотреть результаты →</a>
      <p class="cat-note">Результаты зависят от культуры, почвенно-климатических условий и технологии выращивания.</p>
    </div>
    <div class="proof-col">
      <div class="proof-h">Документы</div>
      <ul class="proof-docs">
        <li>📄 Свидетельства о госрегистрации (СГР)</li>
        <li>📄 Сертификаты соответствия</li>
        <li>📄 Инструкции по применению</li>
        <li>📄 Паспорта безопасности</li>
      </ul>
      <a class="plink">Смотреть документы →</a>
    </div>
  </div>
</div></section>

<!-- 8. СПЕЦИАЛИСТ + ФОРМА (тёмный) -->
<section class="sec sec--dark" id="contact"><div class="wrap">
  <div class="eyebrow">Контакт</div>
  <h2 class="sec-h">Свяжитесь со специалистом</h2>
  <div class="contact">
    <div class="contact-person">
      <div class="cp-top"><div class="av">фото</div>
        <div><h3>Алексей Нилов</h3><div class="role">коммерческий руководитель</div></div>
      </div>
      <p class="cp-text">Поможет подобрать продукцию и рассчитать поставку.</p>
      <div class="cp-lines">
        <a class="cp-tel">☎ +7 960 953-48-88</a>
        <a>✉ nilov@sssaturn.ru</a>
        <a>WhatsApp · Telegram</a>
      </div>
      <a class="plink" style="color:var(--green-soft)">Все контакты и сотрудники →</a>
    </div>
    <div class="contact-form">
      ...
      <div class="form form--dark">
        ...
        <div class="submit">
          <button class="btn o">Получить предложение</button>
        </div>
...
<footer><div class="wrap">
  <div class="fgrid">
    <div class="col"><img src="{logo_white}" alt="Сатурн"><p>Комплексное обеспечение сельхозпроизводителей семенами, удобрениями и средствами защиты растений: подбор, диагностика, поставка и сопровождение.</p></div>
    <div class="col"><b>Разделы</b><a href="#catalog">Каталог</a><a href="#dirs">Подбор</a><a href="#service">Диагностика</a><a href="#service">Агросопровождение</a><a href="#about">О компании</a><a href="#contact">Контакты</a></div>
    <div class="col"><b>Контакты</b><a>☎ +7 960 953-48-88</a><a>☎ +7 963 502-38-55</a><a>WhatsApp · Telegram</a><a>nilov@sssaturn.ru</a><p>г. Барнаул, пр. Ленина 56 / Шевченко 52А, пом. Н13</p></div>
...
<script>
(function(){{
  var tabs=document.querySelectorAll('.need-tab');
  var panels={{prod:document.getElementById('np-prod'),diag:document.getElementById('np-diag'),agro:document.getElementById('np-agro')}};
  tabs.forEach(function(tab){{
    tab.addEventListener('click',function(){{
      tabs.forEach(function(t){{t.classList.remove('on');}});
      tab.classList.add('on');
      var t=tab.getAttribute('data-t');
      Object.keys(panels).forEach(function(k){{ if(panels[k]) panels[k].classList.toggle('on', k===t); }});
    }});
  }});
}})();
</script>
</body></html>'''
open('site-prototype-v5.html','w').write(HTML)
print("written site-prototype-v5.html size", len(HTML))
